...
def test_shell_with_nothing_named_refuses_and_says_how(
    runner, token_vault, fake_launch, monkeypatch
):
    """A subshell outlives the command that opened it: no unnamed whole vault."""
    monkeypatch.delenv("SEKRT_PASSPHRASE")  # and no decryption on the way there
    result = runner.invoke(main, ["shell"])
    assert result.exit_code != 0
    assert "nothing named" in result.output
    assert "-e MY_TOKEN" in result.output
    assert "--all" in result.output
    assert "passphrase" not in (result.output + result.stderr).lower()
    assert fake_launch == []


def test_shell_all_exposes_the_vault_once_acknowledged(
    runner, token_vault, fake_launch, monkeypatch
):
    invoke(runner, "add", "work/github", input="hunter2\nhunter2\n")
    monkeypatch.setattr("click.get_text_stream", lambda *_, **__: _Tty())
    result = invoke(runner, "shell", "--all", "--no-env-files", input="y\n")
    assert result.exit_code == 0
    assert "MY_TOKEN, GITHUB" in result.stderr or "GITHUB, MY_TOKEN" in result.stderr
    assert "exposes all 2 secrets" in result.stderr
    assert "t0ps3cret" not in result.stderr  # the warning never repeats a secret
    env = fake_launch[0]["env"]
    assert env["MY_TOKEN"] == "t0ps3cret"
    assert env["GITHUB"] == "hunter2"


def test_shell_all_declined_exposes_nothing(runner, token_vault, fake_launch, monkeypatch):
    monkeypatch.setattr("click.get_text_stream", lambda *_, **__: _Tty())
    result = runner.invoke(main, ["shell", "--all", "--no-env-files"], input="n\n")
    assert result.exit_code != 0
    assert fake_launch == []


def test_shell_all_without_a_terminal_needs_yes(runner, token_vault, fake_launch):
    """Nothing to answer the prompt: refuse rather than expose the vault anyway."""
    result = runner.invoke(main, ["shell", "--all", "--no-env-files"])
    assert result.exit_code != 0
    assert "--yes" in result.output
    assert fake_launch == []

    result = invoke(runner, "shell", "--all", "--yes", "--no-env-files")
    assert result.exit_code == 0
    assert fake_launch[0]["env"]["MY_TOKEN"] == "t0ps3cret"


def test_shell_naming_a_variable_leaves_the_rest_of_the_vault_alone(
    runner, token_vault, fake_launch
):
    invoke(runner, "add", "work/github", input="hunter2\nhunter2\n")
    result = invoke(runner, "shell", "-e", "MY_TOKEN", "--no-env-files")
...
def test_shell_dry_run_of_all_asks_nothing_and_opens_nothing(
    runner, token_vault, fake_launch
):
    result = invoke(runner, "shell", "--all", "--no-env-files", "-n")
    assert result.exit_code == 0
    assert "MY_TOKEN" in result.output
    assert "t0ps3cret" not in result.output
    assert fake_launch == []
...
@requires_git
def test_shell_with_nothing_named_exposes_this_repos_env_file_only(
    runner, vault_dir, tmp_path, monkeypatch, fake_launch
):
    invoke(runner, "init")
    invoke(runner, "add", "api/my-token", "-t", "api_key", input="t0ps3cret\nt0ps3cret\n")
    ...
    result = invoke(runner, "shell")
    assert result.exit_code == 0
    env = fake_launch[0]["env"]
    assert env["DATABASE_URL"] == "postgres://local"
    assert "MY_TOKEN" not in env  # the vault stays put until --all asks for it
    assert "stored env files only" in result.stderr
